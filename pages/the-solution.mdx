...
To solve the adoption problem we build a non-custodial wallet that ditches the need of EOA (externally owned accounts) and in consecuence the need of storing seedphrases. Allows users to recover their account via a system of guardians (social recovery), pay their tx's with ERC-20 tokens (for example: DAI, USDC or NUARS) and sign using their preffered methods.

What are the benefits of this solution?

- No need to deal with seed-phrases
- User can recover their account in case of device loss or compromise
- Users own their funds. (UNLIKE EXCHANGES OR CUSTODIAL SOLUTIONS)
